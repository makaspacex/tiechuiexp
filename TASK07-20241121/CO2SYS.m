function [DATA,HEADERS,NICEHEADERS]=CO2SYS(T_orgalk,Korg1,Korg2,rat,PAR1,PAR2,PAR1TYPE,PAR2TYPE,SAL,TEMPIN,TEMPOUT,PRESIN,PRESOUT,SI,PO4,pHSCALEIN,K1K2CONSTANTS,KSO4CONSTANTS);
%**************************************************************************
%
...
veclengths=[length(T_orgalk) length(Korg1)...
            length(Korg2) length(rat)...
            length(PAR1)  length(PAR2) length(PAR1TYPE)...
...
T_orgalk                = T_orgalk(:)      ;
Korg1(1:ntps,1)         = Korg1(:)         ;
Korg2(1:ntps,1)         = Korg2(:)         ;
rat(1:ntps,1)         = rat(:)             ;
...
if any(F)
    [PHic(F) FCic(F)] = CalculatepHfCO2fromTATC(T_orgalk,Korg1,Korg2,rat,TAc(F)-PengCorrection(F), TCc(F));
end
F=Icase==13; % input TA, pH
if any(F)
    TCc(F)  = CalculateTCfromTApH(T_orgalk,Korg1,Korg2,rat,TAc(F)-PengCorrection(F), PHic(F));
    FCic(F) = CalculatefCO2fromTCpH(TCc(F), PHic(F));
end
F=Icase==14 | Icase==15; % input TA, (pCO2 or fCO2)
if any(F)
    PHic(F) = CalculatepHfromTAfCO2(T_orgalk,Korg1,Korg2,rat,TAc(F)-PengCorrection(F), FCic(F));
    TCc(F)  = CalculateTCfromTApH(T_orgalk,Korg1,Korg2,rat,TAc(F)-PengCorrection(F), PHic(F));
end
F=Icase==23; % input TC, pH
if any(F)
    TAc(F)  = CalculateTAfromTCpH(T_orgalk,Korg1,Korg2,rat,TCc(F), PHic(F)) + PengCorrection(F);
    FCic(F) = CalculatefCO2fromTCpH(TCc(F), PHic(F));
end
...
Revelleinp              = RevelleFactor(T_orgalk,Korg1,Korg2,rat,TAc-PengCorrection, TCc);
...
F=true(ntps,1); % i.e., do for all samples:
[PHoc FCoc] = CalculatepHfCO2fromTATC(T_orgalk,Korg1,Korg2,rat,TAc-PengCorrection, TCc);
PCoc = FCoc./FugFac;
...
Revelleout              = RevelleFactor(T_orgalk,Korg1,Korg2,rat,TAc, TCc);
...
if any(F)
	...
	if any(FF)
		% Lee, Kim, Byrne, Millero, Feely, Yong-Ming Liu. 2010.	
	 	% Geochimica Et Cosmochimica Acta 74 (6): 1801??811.
...
if any(F)
	...
	% Marine and Freshwater Research, v. 61, p. 139??42.
...
function varargout=CalculatepHfCO2fromTATC(T_orgalk,Korg1,Korg2,rat,TAx, TCx)
global FugFac F;
...
pHx   = CalculatepHfromTATC(T_orgalk,Korg1,Korg2,rat,TAx, TCx); % pH is returned on the scale requested in "pHscale" (see 'constants'...)
...
function varargout=CalculatepHfromTATC(T_orgalk,Korg1,Korg2,rat,TAx,TCx)
global pHScale  WhichKs  WhoseKSO4 sqrSal Pbar RT;
global K0 fH FugFac VPFac ntps TempK logTempK;
global K1 K2 KW KB KF KS KP1 KP2 KP3 KSi;
global TB TF TS TP TSi F;
...
while any(abs(deltapH) > pHTol)
    ...
    BAlk      = TBF.*KBF./(KBF + H);
    Torg1=T_orgalk./(Korg1./(Korg1 + H)+rat*Korg2./(Korg2 + H));
    Torg2=rat*Torg1;
    OrgAlk1 = Torg1.*Korg1./(Korg1 + H);
    OrgAlk2 = Torg2.*Korg2./(Korg2 + H);
    OH        = KWF./H;
    ...
    Residual  = TAx - CAlk - BAlk - OH - PAlk - SiAlk - OrgAlk1 - OrgAlk2 + Hfree + HSO4 + HF;
    % find Slope dTA/dpH;
    % (this is not exact, but keeps all important terms);
    Slope     = ln10.*(TCx.*K1F.*H.*(H.*H + K1F.*K2F + 4.*H.*K2F)./Denom./Denom + BAlk.*H./(KBF + H) + OrgAlk1.*H./(Korg1 + H) +  OrgAlk2.*H./(Korg2 + H)+ OH + H);
...
function varargout=CalculateTCfromTApH(T_orgalk,Korg1,Korg2,rat,TAx, pHx)
global K0 K1 K2 KW KB KF KS KP1 KP2 KP3 KSi;
global TB TF TS TP TSi F
K1F=K1(F);   K2F=K2(F);   KWF=KW(F);
KP1F=KP1(F); KP2F=KP2(F); KP3F=KP3(F); TPF=TP(F);
...
HSO4      = TSF./(1 + KSF./Hfree); %' since KS is on the free scale
HF        = TFF./(1 + KFF./Hfree); %' since KF is on the free scale
CAlk      = TAx - BAlk - OH - PAlk - SiAlk - OrgAlk1 - OrgAlk2 + Hfree + HSO4 + HF;
TCxtemp   = CAlk.*(H.*H + K1F.*H + K1F.*K2F)./(K1F.*(H + 2.*K2F));
varargout{1} = TCxtemp;
end % end nested function


function varargout=CalculatepHfromTAfCO2(T_orgalk,Korg1,Korg2,rat,TAi, fCO2i)
global K0 K1 K2 KW KB KF KS KP1 KP2 KP3 KSi;
global TB TF TS TP TSi F
...
while any(abs(deltapH) > pHTol)
   ...
   OrgAlk2   = Torg2.*Korg2./(Korg2 + H);
    ...
    HSO4      = TSF./(1 + KSF./Hfree); %' since KS is on the free scale
    HF        = TFF./(1 + KFF./Hfree);% ' since KF is on the free scale
    Residual  = TAi - CAlk - BAlk - OH - PAlk - SiAlk - OrgAlk1 - OrgAlk2 + Hfree + HSO4 + HF;
    % '               find Slope dTA/dpH
    % '               (this is not exact, but keeps all important terms):
    Slope     = ln10.*(HCO3 + 4.*CO3 + BAlk.*H./(KBF + H) + OrgAlk1.*H./(Korg1 + H) +  OrgAlk2.*H./(Korg2 + H)+ OH + H);
    deltapH   = Residual./Slope; %' this is Newton's method
    % ' to keep the jump from being too big:
    while any(abs(deltapH) > 1)
        FF=abs(deltapH)>1; deltapH(FF)=deltapH(FF)./2;
...
function varargout=CalculateTAfromTCpH(T_orgalk,Korg1,Korg2,rat,TCi, pHi)
global K0 K1 K2 KW KB KF KS KP1 KP2 KP3 KSi;
global TB TF TS TP TSi F
...
HSO4      = TSF./(1 + KSF./Hfree);% ' since KS is on the free scale
HF        = TFF./(1 + KFF./Hfree);% ' since KF is on the free scale
TActemp     = CAlk + BAlk + OH + PAlk + SiAlk - Hfree - HSO4 - HF+ OrgAlk1 + OrgAlk2;
varargout{1}=TActemp;
end % end nested function
...
function varargout=RevelleFactor(T_orgalk,Korg1,Korg2,rat,TAi,TCi)
% global WhichKs;
...
TCi = TC0 + dTC;
pHc= CalculatepHfromTATC(T_orgalk,Korg1,Korg2,rat,TAi, TCi);
